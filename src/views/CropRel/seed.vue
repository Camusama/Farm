  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="AddVisible"
      ...
      <el-form :model="addForm">
        ...
        <el-form-item label="良种"
                      width="40%">
          <el-input v-model="addForm.name"
                    autocomplete="off" />
        </el-form-item>
        <el-form-item label="特性特征"
                      width="40%">
    ...
    <el-table v-loading="listLoading"
      ...
      <el-table-column label="良种"
                       width="150">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      <el-table-column label="特性特征"
...
import { getIndex, editSeed, addSeed, delSeed } from '@/api/backend/CropRel/seed'
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      this.editForm.point1 = row.feature
      this.editForm.point2 = row.quality
      this.editForm.point3 = row.keypoint
      this.editForm.img = row.img
      this.editId = row.id
      this.EditVisible = true
    },
    submitEdit () {
      editSeed(this.editId, this.editForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将删除该良种信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delSeed(row.id).then(res => {
          if (res.code === 0) {
            Message({
              message: res.msg,
              type: 'success',
