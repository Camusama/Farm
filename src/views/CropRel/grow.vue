  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="AddVisible"
      ...
      <el-form :model="addForm">
        ...
        <el-form-item label="栽培技术"
                      width="40%">
          <el-input v-model="addForm.name"
                    autocomplete="off" />
        </el-form-item>
        <el-form-item label="介绍一"
                      width="40%">
    ...
    <el-table v-loading="listLoading"
      ...
      <el-table-column label="栽培技术"
                       width="150">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      <el-table-column label="介绍一"
...
import { getIndex, editGrow, addGrow, delGrow } from '@/api/backend/CropRel/grow'
...
export default {
  ...
  methods: {
    ...
    submitEdit () {
      editGrow(this.editId, this.editForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将删除该栽培技术信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delGrow(row.id).then(res => {
          if (res.code === 0) {
            Message({
              message: res.msg,
              type: 'success',
