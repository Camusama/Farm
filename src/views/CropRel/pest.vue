...
import { getIndex, editPest, addPest, delPest } from '@/api/backend/CropRel/pest'
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      this.editForm.point1 = row.summary
      this.editForm.point2 = row.feature
    ...
    submitEdit () {
      editPest(this.editId, this.editForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将删除该病害信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delPest(row.id).then(res => {
          if (res.code === 0) {
            Message({
              message: res.msg,
              type: 'success',
