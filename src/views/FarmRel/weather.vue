  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="AddVisible"
      ...
      <el-form :model="addForm">
        ...
        <el-form-item label="灾害"
                      width="40%">
          <el-input v-model="addForm.disaster"
                    autocomplete="off" />
        </el-form-item>
...
import { getIndex, editWeather, addWeather, delWeather } from '@/api/backend/FarmRel/weather'
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      this.editForm.disaster = row.disaster
    ...
    submitEdit () {
      editWeather(this.editId, this.editForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    submitAdd () {
      addWeather(this.addForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将删除该灾害信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delWeather(row.id).then(res => {
          if (res.code === 0) {
            Message({
              message: res.msg,
              type: 'success',
