...
import { getIndex, editFertilizer, addFertilizer, delFertilizer } from '@/api/backend/FarmRel/fertilizer'
...
export default {
  ...
  methods: {
    ...
    submitEdit () {
      editFertilizer(this.editId, this.editForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    submitAdd () {
      addFertilizer(this.addForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
