  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="AddVisible"
      ...
      <el-form :model="addForm">
        ...
        <el-form-item label="安全知识"
                      width="40%">
          <el-input v-model="addForm.safety"
                    autocomplete="off" />
        </el-form-item>
        <el-form-item label="知识一"
                      width="40%">
    ...
    <el-table v-loading="listLoading"
      ...
      <el-table-column label="安全知识"
                       width="150">
        <template slot-scope="scope">
          {{ scope.row.safety }}
        </template>
      </el-table-column>
      <el-table-column label="知识一"
    ...
    <el-dialog :visible.sync="EditVisible"
      ...
      <el-form :model="editForm">
        ...
        <el-form-item label="安全知识"
                      width="40%">
          <el-input v-model="editForm.safety"
                    :disabled="true"
                    autocomplete="off" />
        </el-form-item>
        <el-form-item label="知识一"
...
import { getIndex, editSafety, addSafety, delSafety } from '@/api/backend/FarmRel/safety'
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      this.editForm.safety = row.safety
      this.editForm.point1 = row.point1
      this.editForm.point2 = row.point2
      this.editForm.point3 = row.point3
      this.editForm.img = row.img
      this.editId = row.id
      this.EditVisible = true
    },
    submitEdit () {
      editSafety(this.editId, this.editForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    submitAdd () {
      addSafety(this.addForm).then(res => {
        if (res.code === 0) {
          Message({
            message: res.msg,
            type: 'success',
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将删除该肥料, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delSafety(row.id).then(res => {
          if (res.code === 0) {
            Message({
              message: res.msg,
              type: 'success',
